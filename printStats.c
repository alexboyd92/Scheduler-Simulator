...
void printStats(process a[], os system) {
								...
								printf("\nAverage Wait   Priority   Max wait   Min wait   Total wait\n");
								for (i = 0; i < P_COUNT; i++) {
								...
								for (i = 0; i < P_COUNT; i++) {
																printf("%12.2lf   %8u   %8u   %8u   %8u   %8u\n", avgs[i].average, avgs[i].priority,a[i].waitMax,a[i].waitMin,a[i].waitSum,a[i].waitCount);
								}
}
